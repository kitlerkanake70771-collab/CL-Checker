...
Ch $t2 $y2 'f_chromefix' 'Fix Chrome Policies' $s2.Cat;$y2+=20;Ch $t2 $y2 'f_hvcifix' 'Disable HVCI' $s2.Cat;$y2+=20
Ch $t2 $y2 'f_fastbootfix' 'Disable Fast Boot' $s2.Cat;$y2+=20;Ch $t2 $y2 'f_smartscreenfix' 'Disable SmartScreen' $s2.Cat;$y2+=20
Ch $t2 $y2 'f_exploitfix' 'Disable Exploit Protection' $s2.Cat;$y2+=20;Ch $t2 $y2 'f_gamebarfix' 'Enable Gamebar' $s2.Cat;$y2+=24
...
function ChkDefender{ws 'WINDOWS DEFENDER';$svc=Get-Service -Name 'WinDefend' -ErrorAction SilentlyContinue;if($svc){if($svc.Status -eq 'Running'){wn 'Service RUNNING (should be OFF)'}else{ok "Service: $($svc.Status)"}}else{wn 'Defender removed (custom OS?)'};try{$d=Get-MpComputerStatus -ErrorAction SilentlyContinue;if($d){inf "RealTime: $($d.RealTimeProtectionEnabled)";inf "AV: $($d.AntivirusEnabled)"}}catch{}}
...
function ChkChromePolicies{ws 'CHROME POLICIES';$hp=$false;$own=@('InsecurePrivateNetworkRequestsAllowed','InsecurePrivateNetworkRequestsAllowedForUrls');foreach($p in @('HKLM:\SOFTWARE\Policies\Google\Chrome','HKCU:\SOFTWARE\Policies\Google\Chrome')){if(Test-Path $p){$pr=Get-ItemProperty -Path $p -ErrorAction SilentlyContinue;$bad=$pr.PSObject.Properties|?{$_.Name -notin @('PSPath','PSParentPath','PSChildName','PSDrive','PSProvider','default')+$own};if($bad){wn 'Unwanted policies found';foreach($b in $bad){inf "  $($b.Name) = $($b.Value)"};$hp=$true}}};if(-not $hp){ok 'Clean'};inf 'Auto-fixing SafeBrowsing...';FixChromePolicies}
...
function ChkSmartScreen{ws 'SMARTSCREEN';$ss='HKLM:\SOFTWARE\Policies\Microsoft\Windows\System';$f=$false;if(Test-Path $ss){$h=Get-ItemProperty -Path $ss -ErrorAction SilentlyContinue;if($h.EnableSmartScreen -eq 0){ok 'Disabled by policy';$f=$true}};if(-not $f){wn 'Not disabled';inf 'Auto-fixing...';FixSmartScreen}}
function ChkExploitProt{ws 'EXPLOIT PROTECTION';$ep='HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender Security Center\App and Browser protection';$f=$false;if(Test-Path $ep){$h=Get-ItemProperty -Path $ep -ErrorAction SilentlyContinue;if($h.DisallowExploitProtectionOverride -eq 1){ok 'Disabled by policy';$f=$true}};if(-not $f){wn 'Not disabled';inf 'Auto-fixing...';FixExploitProt}}
function ChkGamebar{ws 'GAMEBAR';$gd='HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\GameDVR';$f=$false;if(Test-Path $gd){$h=Get-ItemProperty -Path $gd -ErrorAction SilentlyContinue;if($h.AppCaptureEnabled -eq 1){ok 'Enabled';$f=$true}};if(-not $f){wn 'Not enabled';inf 'Auto-fixing...';FixGamebar};try{$pk=Get-AppxPackage -Name 'Microsoft.XboxGamingOverlay*' -ErrorAction SilentlyContinue;if($pk){ok "App installed"}else{wn 'App not installed'}}catch{}}
function ChkOSIntegrity{ws 'OS INTEGRITY';$svc=Get-Service -Name 'WinDefend' -ErrorAction SilentlyContinue;if(-not $svc){wn 'Defender service missing - OS appears modified'}else{ok 'OS appears unmodified'};dism /Online /Cleanup-Image /CheckHealth 2>&1|Out-Null;if($LASTEXITCODE -eq 0){ok 'DISM: healthy'}else{wn "DISM exit: $LASTEXITCODE"}}
...
function FixSyncTime{ws 'SYNC TIME';bgr {net stop w32time 2>&1;w32tm /unregister 2>&1;w32tm /register 2>&1;sc config w32time start=auto 2>&1;net start w32time 2>&1;w32tm /resync 2>&1;w32tm /resync 2>&1} 'Sync Time'}
...
function FixSmartScreen{ws 'DISABLE SMARTSCREEN';$ss='HKLM:\SOFTWARE\Policies\Microsoft\Windows\System';try{if(-not(Test-Path $ss)){New-Item -Path $ss -Force -ErrorAction Stop|Out-Null};Set-ItemProperty -Path $ss -Name 'EnableSmartScreen' -Value 0 -Type DWord -Force;ok 'SmartScreen disabled'}catch{er "Failed"}}
function FixExploitProt{ws 'DISABLE EXPLOIT PROT';$ep='HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender Security Center\App and Browser protection';try{if(-not(Test-Path $ep)){New-Item -Path $ep -Force -ErrorAction Stop|Out-Null};Set-ItemProperty -Path $ep -Name 'DisallowExploitProtectionOverride' -Value 1 -Type DWord -Force;ok 'ExploitProt disabled'}catch{er "Failed"}}
function FixGamebar{ws 'ENABLE GAMEBAR';$gd='HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\GameDVR';try{if(-not(Test-Path $gd)){New-Item -Path $gd -Force -ErrorAction Stop|Out-Null};Set-ItemProperty -Path $gd -Name 'AppCaptureEnabled' -Value 1 -Type DWord -Force;ok 'Gamebar enabled'}catch{er "Failed"}}
function FixFastBoot{ws 'DISABLE FAST BOOT';$fb='HKLM:\SYSTEM\CurrentControlSet\Control\Session Manager\Power';try{Set-ItemProperty -Path $fb -Name 'HiberbootEnabled' -Value 0 -Type DWord -Force -ErrorAction Stop;ok 'Fast Boot disabled'}catch{er "Failed"}}
...
$ACT=@{
    ...
    c_fastboot   =@{C=${function:ChkFastBoot};        F=${function:FixFastBoot};       L='FastBoot'}
    c_smartscreen=@{C=${function:ChkSmartScreen};     F=${function:FixSmartScreen};    L='SmartScrn'}
    c_exploit    =@{C=${function:ChkExploitProt};     F=${function:FixExploitProt};    L='Exploit'}
    c_gamebar    =@{C=${function:ChkGamebar};         F=${function:FixGamebar};        L='Gamebar'}
    ...
    f_fastbootfix=@{C=$null;                          F=${function:FixFastBoot};       L='FastBoot'}
    f_smartscreenfix=@{C=$null;                       F=${function:FixSmartScreen};    L='SmartScr'}
    f_exploitfix =@{C=$null;                          F=${function:FixExploitProt};    L='Exploit'}
    f_gamebarfix =@{C=$null;                          F=${function:FixGamebar};        L='Gamebar'}
